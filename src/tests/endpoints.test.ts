...
beforeAll(async () => {
  await fs.unlink("db/database.sqlite", () => null);
...
describe("/api/users", () => {
  ...
  test("throws an error when trying to create a user without username and/or password", async () => {
  ...
  test("throws an error when trying to create a user with the same username", async () => {
    const response = await supertest(app).post("/api/users").send({
      username: "test_user",
      password: "test_password",
    });
...
describe("/api/rooms", () => {
  ...
  test("throws an error when no token is provided", async () => {
  ...
  test("returns an empty list of rooms", async () => {
  ...
  test("owner of the room is the user who created it", async () => {
  ...
  test("returns a list with newly created room", async () => {
    const response = await supertest(app).get("/api/rooms").set("Authorization", `Bearer ${token}`);
    expect(response.status).toBe(200);
